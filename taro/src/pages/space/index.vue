    ...
    <view class="card">
      ...
      <view class="card-tags">
        <text v-for="(tag, ti) in space.tags" :key="tag">
          {{ tag
          }}<text v-if="ti < space.tags.length - 1" class="tag-divider">
            ·
          </text>
        </text>
      </view>
      ...
      <view v-if="roomList.length > 0" class="room-list">
        ...
        >
          <view class="room-left">
            ...
            <text v-if="!getRoomAvailable(room)" class="room-item-tag"
              >已约满</text
            >
          </view>
      ...
      <view class="info-row">
        <view class="info-left">
          <view class="info-value-row">
            <image
              class="info-icon"
              src="@/assets/icons/icon-location@2x.png"
              mode="aspectFill"
            />
  ...
  <BottomBar v-if="space" justify="between" shadow>
    <view class="bar-actions">
      ...
      <view class="bar-action" @tap="onFavorite">
        <image
          class="bar-action-icon-img"
          :src="favorited ? iconFavorited : iconFavorite"
          mode="aspectFill"
        />
...
const UNAVAIL_ROOM_IDS = new Set([
  'cmrnjdy9x00015me9lbsj4txr',
  'cmrnje5d800065me9pzckx2jx'
])
