    ...
    <view class="card">
      ...
      <view class="form-item no-border">
        ...
        <text class="duration-value" v-if="timeRange.start && timeRange.end">
          {{ timeRange.start }} - {{ timeRange.end }}（共{{
            selectedDuration
          }}小时）
        </text>
        ...
        <text class="duration-value hint" v-else> 选择时间段后自动计算 </text>
      </view>
      ...
      <view class="form-item no-border">
        ...
        <view class="stepper">
          <view
            class="stepper-btn"
            :class="{ disabled: checkoutInfo.partySize <= 1 }"
            @tap="decreasePartySize"
            >−</view
          >
          <text class="stepper-value">{{ checkoutInfo.partySize }}</text>
          <view
            class="stepper-btn"
            :class="{ disabled: isMaxParty }"
            @tap="increasePartySize"
            >+</view
          >
        </view>
      </view>
    ...
    <view class="card">
      ...
      <view class="price-row">
        ...
        <text class="price-value">
          <template v-if="timeRange.start && timeRange.end"
            >¥{{ checkoutInfo.roomPrice
            }}<template v-if="checkoutInfo.roomPrice > 0">
              × {{ selectedDuration }}小时</template
            ></template
      ...
      <view class="price-row">
        ...
        <text class="price-value"
          ><template v-if="timeRange.start && timeRange.end"
            >¥{{ checkoutInfo.serviceFee }}</template
          ><template v-else>¥0</template></text
        >
      </view>
    ...
    <BottomBar shadow>
      <view class="bottom-price">
        ...
        <text class="bottom-total">{{
          timeRange.start && timeRange.end ? `¥${total}` : '—'
        }}</text>
      </view>
      <SubmitButton
        text="提交订单"
